...
GameState = collections.namedtuple(
  'GameState',
  ['turn', 'player_hp', 'player_mana', 'enemy_hp',
   'shield_turns', 'poison_turns', 'recharge_turns'])
INITIAL_STATE = GameState(PLAYER_TURN, 50, 500, enemy_hp, 0, 0, 0)
...
def solve(state):
  ...
  if state.turn == ENEMY_TURN:
    ...
    # Cast Drain
    best = min(best, 73 + solve(GameState(
      1 - state.turn,
    ...
    if state.shield_turns <= 1:
      best = min(best, 113 + solve(GameState(
        1 - state.turn,
        state.player_hp - (1 if PART == 2 else 0),
    ...
    if state.poison_turns <= 1:
      best = min(best, 173 + solve(GameState(
        1 - state.turn,
        state.player_hp - (1 if PART == 2 else 0),
    ...
    if state.recharge_turns <= 1:
      best = min(best, 229 + solve(GameState(
        1 - state.turn,
        state.player_hp - (1 if PART == 2 else 0),
...
print('part1:', solve(INITIAL_STATE))  # 953

PART = 2
mem = {}
print('part2:', solve(INITIAL_STATE))  # 1289
