...
for line in inp.splitlines():
    ...
    for range_str in rest.split(','):
        mi, ma = range_str[2:].split('..')
        cmd.append((int(mi), int(ma)))
    cmds.append(cmd)
# print(cmds)
...
def part1():
    on = set()
    for cmd in cmds:
        turn_on, (xmin, xmax), (ymin, ymax), (zmin, zmax) = cmd
        xmin = max(-50, xmin)
        xmax = min(50, xmax)
        ymin = max(-50, ymin)
        ymax = min(50, ymax)
...
def linear_intersection(range1, range2):
    ...
    if min2 <= min1 < max2:
        return min1, min(max2, max1)
    if min2 < max1 <= max2:
        return max(min2, min1), max1
    if min1 <= min2 < max1:
        return min2, min(max1, max2)
    if min1 < max2 <= max1:
...
# Set of cubes (xrange, yrange, zrange)
on = set()
for cmd in cmds:
    turn_on, xrange, yrange, zrange = cmd
